...
const outputDir = path.resolve('debug', 'gemini-fast-pack-output');
...
async function installGeminiSkills() {
  return requestJson(`${API_BASE}/api/site-packs/gemini.google.com/install-skills`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({}),
  });
...
async function main() {
  ...
  await navigate(tabId, 'https://gemini.google.com');
  await new Promise((resolve) => setTimeout(resolve, 5000));

  const room = await getSiteRoom(tabId);
  const suggestions = await getSuggestions(tabId);
  const installResult = await installGeminiSkills();
  ...
  const hasSendPromptSuggestion = suggestions.suggestions?.some(
    (item) => item.name === 'gemini_send_prompt' && item.safe === true
  );

  const authRequired =
    room.reason === 'AUTH_REQUIRED' ||
    suggestions.reason === 'AUTH_REQUIRED' ||
    room.room === 'gemini_auth' ||
    suggestions.suggestions?.some((item) =>
      item.guarded === true && /requires login|login is required/i.test(String(item.reason || ''))
    );

  let sendPromptResult = null;
  if (promptBox.found && sendButton.found) {
    sendPromptResult = await runSkill(tabId, {
      skill: 'gemini_send_prompt',
      inputs: {
        prompt: 'Say exactly: GlassBox Gemini fast pack test.',
      },
      stopOnFailure: true,
    }, true);
  }
  ...
  const roomOk =
    room.room === 'gemini_auth' ||
    room.room === 'gemini_home' ||
    room.room === 'gemini_chat' ||
    authRequired;
