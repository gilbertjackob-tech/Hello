...
const outputDir = path.resolve('debug', 'run-micro-skill-output');
...
async function saveSkill() {
  return requestJson(`${API_BASE}/api/skills`, {
    ...
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      name: 'youtube_search_replay_test',
      queryPattern: 'search youtube replay test',
      steps: [
        {
...
async function main() {
  ...
  const saveResult = await saveSkill();

  const runResult = await runSkill(tabId, {
    skill: 'youtube_search_replay_test',
    inputs: {
      query: 'database lecture'
    },
    stopOnFailure: true,
  });
  ...
  const passed =
    ...
    Boolean(runResult.ok) &&
    Boolean(runResult.chainResult?.ok) &&
    runResult.chainResult?.completedCount === 2 &&
    Number(runResult.updatedSkill?.success_count || 0) >= 1;
  ...
  const summary = [
    ...
    `completedCount: ${runResult.chainResult?.completedCount}`,
    `successCount: ${runResult.updatedSkill?.success_count}`,
    `result: ${resultPath}`,
  ].join('\n');
