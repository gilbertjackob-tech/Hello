...
const outputDir = path.resolve('debug', 'whatsapp-send-file-self-output');
...
async function main() {
  ...
  fs.writeFileSync(filePath, `GlassBox WhatsApp file self-test ${Date.now()}\n`, 'utf8');
  ...
  const result = await requestJson(`${API_BASE}/api/whatsapp/send-file`, {
    ...
    body: JSON.stringify({
      ...
      chat,
      filePath,
      caption: 'GlassBox file self-test',
