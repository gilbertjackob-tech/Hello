...
const outputDir = path.resolve('debug', 'whatsapp-send-message-self-output');
...
async function main() {
  ...
  const message = `GlassBox self-test ${Date.now()}`;
  const result = await requestJson(`${API_BASE}/api/whatsapp/send-message`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      tabId,
  ...
  const passed = result.sent === true && result.reason === 'MESSAGE_SENT';
  const payload = {
