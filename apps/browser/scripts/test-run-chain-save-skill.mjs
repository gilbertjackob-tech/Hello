...
const outputDir = path.resolve('debug', 'run-chain-save-skill-output');
...
async function main() {
  ...
  const chainResult = await runChain(tabId, {
    stopOnFailure: true,
    saveAsSkill: {
      name: 'youtube_search_from_chain',
      queryPattern: 'search youtube from chain'
    },
    steps: [
      {
  ...
  const found = Array.isArray(skills) && skills.some((skill) => skill.name === 'youtube_search_from_chain');

  const passed =
    Boolean(chainResult.ok) &&
    Boolean(chainResult.savedSkill) &&
    found;
  ...
  const summary = [
    ...
    `savedSkill: ${Boolean(chainResult.savedSkill)}`,
    `found: ${found}`,
    `result: ${resultPath}`,
  ].join('\n');
