...
const outputDir = path.resolve('debug', 'whatsapp-browser-identity-output');
...
async function main() {
  ...
  const identity = await evaluate(tabId, `
    (() => ({
      url: location.href,
      ...
      unsupported:
        /update google chrome|works with google chrome|update chrome/i.test(document.body?.innerText || '')
    }))()
  `);

  const unsupported = Boolean(identity?.result?.unsupported);
  const passed = unsupported === false;

  const payload = {
    runAt: new Date().toISOString(),
    tabId,
    passed,
    identity: identity?.result || null,
  };
  ...
  const summary = [
    ...
    `url: ${identity?.result?.url || ''}`,
    `title: ${identity?.result?.title || ''}`,
    `result: ${resultPath}`,
  ].join('\n');
