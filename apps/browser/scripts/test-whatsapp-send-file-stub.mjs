...
const outputDir = path.resolve('debug', 'whatsapp-send-file-stub-output');
...
async function main() {
  ...
  fs.writeFileSync(filePath, `GlassBox WhatsApp file stub test ${Date.now()}\n`, 'utf8');

  const chat = process.env.WHATSAPP_TEST_CHAT || 'Hasnat (You)';
  const result = await requestJson(`${API_BASE}/api/whatsapp/send-file`, {
    ...
    body: JSON.stringify({
      ...
      chat,
      filePath,
      caption: 'GlassBox file stub test',
      allowExternalSend: false,
    }),
  });

  const passed = result.ok === false && result.reason === 'FILE_UPLOAD_NOT_IMPLEMENTED_YET' && result.sent === false;
  const payload = {
