...
const APP_MODE_STORAGE_KEY = 'gb-app-mode';
...
function getInitialAppMode(): AppMode {
  ...
  return saved === 'hello' || saved === 'browser' ? saved : 'browser';
}
...
export default function App() {
  ...
  const switchAppMode = (mode: AppMode) => {
    ...
    void fetch(`/api/hello/mode/${mode}`, { method: 'POST' }).catch(() => undefined);
    requestAnimationFrame(() => syncBrowserViewBounds(activeTabId, mode));
  };
  ...
  const syncBrowserViewBounds = (tabIdToSync: string | null = activeTabId, mode: AppMode = appModeRef.current) => {
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => syncBrowserViewBounds(activeTabId, appMode));
  }, [appMode, activeTabId]);
  ...
  }, [activeTabId, tabs.length, enabledPanels.length, appMode]);
  ...
  return (
    <div className="grid h-full w-full min-h-0 min-w-0 grid-rows-[auto_minmax(0,1fr)] overflow-hidden border border-gb-border bg-gb-bg font-sans text-sm text-gb-text">
      <header className="flex min-w-0 shrink-0 flex-col overflow-visible border-b border-gb-border">
        <div className="drag-region flex h-10 min-w-0 items-center gap-2 border-b border-gb-border bg-gb-surface px-2 py-1">
          {appMode === 'browser' ? (
            ...
            <div className="flex min-w-0 items-center gap-2 px-2 text-[11px] font-semibold uppercase text-gb-text-dim">
              <Box size={14} className="text-gb-accent-success" />
              Hello
            </div>
          )}
          ...
          <button
            type="button"
            onClick={() => switchAppMode(appMode === 'browser' ? 'hello' : 'browser')}
            className="no-drag mr-32 inline-flex h-8 shrink-0 items-center rounded-md border border-gb-border bg-gb-surface-bright px-3 text-[11px] font-semibold text-gb-text transition-colors hover:bg-gb-bg"
            aria-label={appMode === 'browser' ? 'Open Hello' : 'Return to GlassBox'}
            title={appMode === 'browser' ? 'Open Hello' : 'Return to GlassBox'}
          >
            {appMode === 'browser' ? 'Hello' : 'GlassBox'}
          </button>
        </div>

        {appMode === 'browser' && <div className="relative z-50 flex min-w-0 items-center gap-2 overflow-visible border-t border-gb-border bg-gb-bg p-2">
      ...
      <div className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden">
        <iframe
          ...
          title="Hello"
          className={`${appMode === 'hello' ? 'relative flex h-full min-h-0 w-full min-w-0 flex-1' : 'pointer-events-none absolute h-0 w-0 opacity-0'} border-0 bg-white`}
          aria-hidden={appMode !== 'hello'}
        />

        <main className={`${appMode === 'browser' ? 'relative flex min-h-0 min-w-0 flex-1' : 'absolute h-0 w-0'} flex-col overflow-hidden bg-black`}>
          <div
            ref={browserViewRef}
            className="relative flex h-full min-h-0 w-full min-w-0 flex-1 overflow-hidden bg-black"
          >
            {appMode === 'browser' && !activeTabId && <div className="flex h-full items-center justify-center"></div>}
            {appMode === 'browser' && activeTabId && (
