...
import { mountHello } from '../../../hello/src/server/mountHello.js';
import db from './memoryDb.js';
import { exportFullProfileBackup, importFullProfileBackup } from './profileBackupService.js';
import { profileStore } from './profileStore.js';
import { BROWSER_ROOT, HELLO_DATA_DIR, HELLO_DB_PATH, HELLO_ROOT, HELLO_UPLOADS_DIR } from '../server/paths.js';
...
export function startApiServer(port: number = 3000): Promise<void> {
  ...
  serverPromise = (async () => {
    ...
    app.get('/api/hello/status', (_req, res) => {
      res.json({
        ok: true,
        mounted: true,
        basePath: '/hello',
        apiPath: '/hello/api',
        socketPath: '/hello/socket.io',
        uploadsPath: '/hello/uploads',
      });
    });

    app.post('/api/hello/open', (_req, res) => {
      res.json({ ok: true, mode: 'hello', url: 'http://127.0.0.1:3000/hello' });
    });

    app.post('/api/hello/mode/browser', (_req, res) => {
      res.json({ ok: true, mode: 'browser' });
    });

    app.post('/api/hello/mode/hello', (_req, res) => {
      res.json({ ok: true, mode: 'hello', url: 'http://127.0.0.1:3000/hello' });
    });
    ...
    app.get('/api/tabs/:id/site-room/suggestions', async (req, res) => {
      try {
        res.json(await vlmPageApi.getSiteRoomSuggestions(req.params.id));
      } catch (error: any) {
        errorResponse(res, error);
      }
    });
    ...
    await mountHello(app, httpServer, {
      ...
      apiPath: '/hello/api',
      socketPath: '/hello/socket.io',
      uploadsPath: HELLO_UPLOADS_DIR,
      dbPath: HELLO_DB_PATH,
      dataDir: HELLO_DATA_DIR,
      helloRoot: HELLO_ROOT,
    });

    if (process.env.NODE_ENV !== 'development') {
      app.use(express.static(path.join(BROWSER_ROOT, 'dist')));
      app.get('*', (_req, res) => {
        res.sendFile(path.join(BROWSER_ROOT, 'dist', 'index.html'));
      });
    }
