...
const mediaPermissionSessions = new WeakSet<any>();

function isTrustedHelloPermissionUrl(rawUrl: string) {
  try {
    const url = new URL(rawUrl);
    const isHelloRoute = url.pathname === '/hello' || url.pathname.startsWith('/hello/');
...
function installMediaPermissionHandler(partitionSession: any) {
  if (!partitionSession || mediaPermissionSessions.has(partitionSession)) {
    return;
  }

  mediaPermissionSessions.add(partitionSession);

  if (typeof partitionSession.setPermissionRequestHandler !== 'function') {
    return;
  }
...
class TabManager {
  ...
  createTabSync(profileId: string = 'default', initialUrl: string = DEFAULT_LANDING_URL): string {
    ...
    installMediaPermissionHandler(partitionSession);
